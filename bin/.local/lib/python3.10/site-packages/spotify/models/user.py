...
class WrappedUser:
    ...
    async def __aenter__(self):
        if self.user is None:
            self.user = user = await self.coro

        return await user.__aenter__()

    async def __aexit__(self, exc_type, exc_value, traceback):
        await self.user.__aexit__(exc_type, exc_value, traceback)
...
class User(URIBase, AsyncIterable):  # pylint: disable=too-many-instance-attributes
    ...
    def from_token(
        ...
        return WrappedUser(coro=constructor())
